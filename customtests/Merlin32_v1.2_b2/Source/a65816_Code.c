...
static int GetOperandNbByte(char *,struct source_line *,int,int *,char *,struct omf_segment *);
...
static void BuildOneCodeLineOpcode(struct source_line *current_line, struct omf_segment *current_omfsegment)
{
  ...
  if(!my_stricmp(current_line->opcode_txt,"ADC") || !my_stricmp(current_line->opcode_txt,"ADCL"))
  ...
  else if(!my_stricmp(current_line->opcode_txt,"JML"))
    {
      if(address_mode == AM_ABSOLUTE || address_mode == AM_DIRECT_PAGE || address_mode == AM_ABSOLUTE_LONG) /* On fait passer le $00 ou le $000000 pour un $0000 */
        {
          current_line->nb_byte = 3;
          current_line->opcode_byte = 0xDC;
          current_line->address_mode = AM_ABSOLUTE;     /* AM_ABSOLUTE Indirect Long in Bank $00 */
        }
    }
  else if(!my_stricmp(current_line->opcode_txt,"JMP") || !my_stricmp(current_line->opcode_txt,"JMPL"))
    {
  ...
  else if(!my_stricmp(current_line->opcode_txt,"STX"))
    {
      ...
      else if(address_mode == AM_DIRECT_PAGE_INDEXED_Y || address_mode == AM_ABSOLUTE_INDEXED_Y)             /* Le STX $0000,X peut aussi être intepreté comme STX $00,X */        
      //else if(address_mode == AM_DIRECT_PAGE_INDEXED_Y)
        {
          current_line->nb_byte = 2;
...
static void BuildOneCodeLineOperand(struct source_line *current_line, int *has_error_rtn, struct omf_segment *current_omfsegment, struct omf_project *current_omfproject)
{
  ...
  int operand_size, operand_value, modif, delta, is_reloc_1, is_reloc_2, is_mvp_mvn, line_address;
  ...
  if(current_line->address_mode == AM_PC_RELATIVE)
          ...
          if(delta > 128)
            {
              ...
              if(*has_error_rtn == 0)
                {
                  sprintf(param->buffer_latest_error,"Bad Jump for instruction '%s  %s' (line %d, file '%s') : Too long (%d bytes, max is 128)",
                          current_line->opcode_txt,current_line->operand_txt,current_line->file_line_number,current_line->file->file_name,delta);
                  *has_error_rtn = 1;
                }
              /* Valeur en erreur */
              operand_value = 0x00;
          ...
          if(delta > 127)
            {
              ...
              if(*has_error_rtn == 0)
                {              
                  sprintf(param->buffer_latest_error,"Bad Jump for instruction '%s  %s' (line %d, file '%s') : Too long (%d bytes, max is 127)",
                          current_line->opcode_txt,current_line->operand_txt,current_line->file_line_number,current_line->file->file_name,delta);
                  *has_error_rtn = 1;
                }
              /* Valeur en erreur */
  ...
  else if(current_line->address_mode == AM_PC_RELATIVE_LONG)
    {
      ...
      if((current_line->address+3) > operand_value)
        {
          /* On saute vers le Haut */
          delta = (current_line->address+3) - operand_value;
          if(delta > 32768)
            {
              /* On va devoir faire un saut dans l'autre sens et utiliser le fait qu'on reste dans le banc (Wrap) */
      ...
      if(is_reloc_1 == 1)
        {
          ...
          if(current_line->is_fixed_address == 1 && current_line->address != current_line->global_address)
...
{
  int i, j, nb_byte, nb_byte_left, nb_byte_right, is_address, is_address_only_operand;
  char *next_char;
  char operand[1024];

  /* Est-ce un opcode qui n'accepte que des Adresses comme operand (ASL $0300 vs LDA #$10) */
  is_address_only_operand = IsAddressOnlyOpcode(current_line->opcode_txt);
    ...
    if(!my_stricmp(&operand[strlen(operand)-2],",X"))
      {
        ...
        /** $@,X $@@,X ou $@@@,X **/
        nb_byte = GetOperandNbByte(operand,current_line,1,&is_address,error_buffer_rtn,current_omfsegment);    /* Force Address */
          ...
          if(operand[0] == '(' && operand[strlen(operand)-3] == ','&& toupper(operand[strlen(operand)-2]) == 'S' && operand[strlen(operand)-1] == ')')        
            {
              ...
              nb_byte = GetOperandNbByte(operand,current_line,0,&is_address,error_buffer_rtn,current_omfsegment);
              if(nb_byte == 1)
  ...
  if(next_char != NULL)
    {
      ...
      *next_char = '\0';
      ...
      nb_byte_left = GetOperandNbByte(operand,current_line,1,&is_address,error_buffer_rtn,current_omfsegment);      /* Force Address */
      nb_byte_right = GetOperandNbByte(next_char+1,current_line,1,&is_address,error_buffer_rtn,current_omfsegment); 
  ...
  /**  $Absolute  **/
  /*****************/
  if(is_address_only_operand == 1)
    {
      nb_byte = GetOperandNbByte(operand,current_line,1,&is_address,error_buffer_rtn,current_omfsegment); 
      if(nb_byte > 0)
        {
          /** On doit choisir entre $@, $@@ et $@@@ **/
          if(nb_byte == 1 && current_line->no_direct_page == 0)
...
/****************************************************************************************************************/
static int GetOperandNbByte(char *operand, struct source_line *current_line, int force_address, int *is_address_rtn, char *buffer_error_rtn, struct omf_segment *current_omfsegment)
{
  ...
  *is_address_rtn = 0;
  ...
  if(!my_stricmp(current_line->opcode_txt,"ADCL") || !my_stricmp(current_line->opcode_txt,"ANDL") || !my_stricmp(current_line->opcode_txt,"CMPL") ||
     !my_stricmp(current_line->opcode_txt,"EORL") || !my_stricmp(current_line->opcode_txt,"JSL")  || !my_stricmp(current_line->opcode_txt,"JMPL") || 
  ...
  /* L'Operand est t'elle une adresse ? (Label1+1 est une adresse, (Label1-Label2) est une valeur, #toto est une valeur) */
  if(nb_address > 0)
    if(nb_address % 2 == 0)
      is_address = 0;
  if(has_dash == 1)
  ...
  /* On va forcer la reconnaissance d'un adresse (pour les ,X ,Y () [] ou les opcodes n'acceptant que les adresses) */
  if(force_address == 1 && is_address == 0)
    is_address = 1;
