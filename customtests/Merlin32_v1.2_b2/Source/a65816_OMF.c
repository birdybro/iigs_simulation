...
int BuildOMFFile(char *output_folder_path, struct omf_project *current_omfproject)
{
  ...
  /* Les binaires OMF (mono ou multi-segment) n'ont qu'un seul fichier (on prend le premier file) */
  ...
  for(offset=0,current_omfsegment=current_omffile->first_segment; current_omfsegment; current_omfsegment=current_omfsegment->next)
    {
      /* Header */
  ...
  /*******************************************/  
  /* Chemin du fichier */
  sprintf(file_path,current_omffile->dsk_path);
...
int BuildExpressLoadSegment(struct omf_project *current_omfproject)
{
  ...
  /* Pour les binaires OMF qui vont recevoir l'ExpressLoad, il n'y a qu'un seul fichier */
  current_omffile = current_omfproject->first_file;

  /** Allocation mémoire **/
  xpress_omfsegment = mem_alloc_omfsegment(xpress_name);
  if(xpress_omfsegment == NULL)
    return(1);

  /* Attachement en 1ère position */
  current_omffile->nb_segment++;
  xpress_omfsegment->next = current_omffile->first_segment;
  current_omffile->first_segment = xpress_omfsegment;
  ...
  one_word = (WORD) (current_omffile->nb_segment - 2);
...
/*******************************************************************************/
void UpdateFileInformation(char *output_file_path, char *output_file_name, struct omf_project *current_omfproject)
{
  FILE *fd;
  char *next_sep;
  int i, nb_line;
  ...
  GetFolderFromPath(output_file_path,output_folder_path);
...
{
  struct omf_file *current_omffile;
  struct omf_file *next_omffile;

  if(current_omfproject)
    {
      for(current_omffile=current_omfproject->first_file; current_omffile; )
        {
          next_omffile = current_omffile->next;
          mem_free_omffile(current_omffile);
          current_omffile = next_omffile;
        }
...
/************************************************************************/
/*  mem_alloc_omffile() :  Allocation mémoire de la structure omf_file. */
/************************************************************************/
struct omf_file *mem_alloc_omffile(char *dsk_name)
{
  struct omf_file *current_omffile;

  /* Allocation mémoire */
  current_omffile = (struct omf_file *) calloc(1,sizeof(struct omf_file));
  if(current_omffile == NULL)
    return(NULL);

  /* Init */
  current_omffile->dsk_path = BuildProjectFilePath(dsk_name,"DSK");
  if(current_omffile->dsk_path == NULL)
    {
      mem_free_omffile(current_omffile);
      return(NULL);
    }
  current_omffile->dsk_name = BuildProjectFileName(current_omffile->dsk_path);
  if(current_omffile->dsk_name == NULL)
    {
      mem_free_omffile(current_omffile);
      return(NULL);
    }
  current_omffile->org_address = 0xFFFFFFFF;
  current_omffile->file_size   = 0;

  /* Renvoie la structure */
  return(current_omffile);
}


/***********************************************************************/
/*  mem_free_omffile() :  Libération mémoire de la structure omf_file. */
/***********************************************************************/
void mem_free_omffile(struct omf_file *current_omffile)
{
  struct omf_segment *current_omfsegment;
  ...
  if(current_omffile)
    {
      if(current_omffile->dsk_name)
        free(current_omffile->dsk_name);

      if(current_omffile->dsk_path)
        free(current_omffile->dsk_path);

      for(current_omfsegment=current_omffile->first_segment; current_omfsegment; )
        {
...
/******************************************************************************/
struct omf_segment *mem_alloc_omfsegment(char *segment_name)
{
  struct omf_segment *current_omfsegment;
  ...
  current_omfsegment->segment_name    = strdup(segment_name);
  ...
  if(current_omfsegment)
    {
      ...
      my_Memory(MEMORY_FREE_SOURCE_FILE,NULL,NULL,current_omfsegment);
