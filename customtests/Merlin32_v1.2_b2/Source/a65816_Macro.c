...
static void LoadOneMacroFile(char *,struct source_line *,struct omf_segment *);
...
/***********************************************************************/
void LoadSourceMacroFile(struct omf_segment *current_omfsegment)
{
      ...
      if(!my_stricmp(current_line->opcode_txt,"USE") && strlen(current_line->operand_txt) > 0)
        {
          ...
          if(!IsMacroFile(current_line->operand_txt))
            continue;

          /** Charge le fichier Macro **/
          printf("        - %s\n",current_line->operand_txt);
          LoadOneMacroFile(current_line->operand_txt,current_line,current_omfsegment);
        }
    }
}
...
/*********************************************************/
static void LoadOneMacroFile(char *file_name, struct source_line *macro_line, struct omf_segment *current_omfsegment)
{
  ...
  if(file_path != NULL)
    {
      strcpy(macro_file_path,file_path);
      free(file_path);
    }
  else
    {
      sprintf(param->buffer_error,"Impossible to allocate memory to build Macro file path '%s'",file_name);
      my_RaiseError(ERROR_RAISE,param->buffer_error);
    }

  /* Chargement du fichier en mémoire (on recherche dans le dossier Library) */
  data = (char *) LoadTextFileData(macro_file_path,&data_size);
  if(data == NULL)
    {
      if(macro_line != NULL)
        ...
        sprintf(param->buffer_error,"Impossible to load Macro file '%s'",macro_file_path);
      my_RaiseError(ERROR_RAISE,param->buffer_error);
    }
...
/******************************************************************/
int IsMacroFile(char *file_name)
{
  int found;
  char *file_path;
