...
{
  jmp_buf context;
  int verbose_mode, context_value, error;
  ...
  /* Message Information */
  printf("%s v 1.2 beta 2, (c) Brutal Deluxe 2011-2025\n",argv[0]);

  /* Vérification des paramètres */
  if(argc != 3 && argc != 4)
    {
      printf("  Usage : %s [-V] <macro_folder_path> <link_file_path|source_file_path>.\n",argv[0]);
      return(1);
    }
  ...
  if(context_value)
    {
      ...
      my_Memory(MEMORY_GET_CURR_ERROR_SEGMENT,&current_omfsegment,NULL,NULL);
  ...
  param = mem_alloc_param(macro_folder_path);
  if(param == NULL)
  ...
  /** Prépare le chemin des fichiers Output : Extrait le Dossier contenant le fichier Master ou Link **/
  BuildOutputFolderPath(source_file_path,param->project_folder_path);

  /*** Assemble et Link tous les fichiers du projet ***/
  error = AssembleLink65c816(source_file_path,verbose_mode);
