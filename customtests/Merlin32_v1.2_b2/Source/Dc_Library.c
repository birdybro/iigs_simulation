...
void my_File(int code, void *data)
{
  ...
  switch(code)
    {
      case FILE_INIT_DIRECTORY :
        for(i=0; i<256; i++)
          hFile_tab[i] = 0;
        break;

      case FILE_CLOSE_DIRECTORY :
...
void my_Memory(int code, void *data, void *value, struct omf_segment *current_omfsegment)
{
  ...
  static struct omf_segment *curr_error_omfsegment;
  ...
  switch(code)
    {
      ...
      /*****************************************************/
      /*  Structure OMF Segment courrante en cas d'erreur  */
      /*****************************************************/
      case MEMORY_SET_CURR_ERROR_SEGMENT :
        curr_error_omfsegment = (struct omf_segment *) data;
        break;

      case MEMORY_GET_CURR_ERROR_SEGMENT :
        *((struct omf_segment **) data) = curr_error_omfsegment;
        break;
...
/****************************************************************/
/*  GetNameFromPath() :  Extrait le nom de fichier d'un chemin. */
/****************************************************************/
void GetNameFromPath(char *file_path, char *file_name_rtn)
{
  int i;

  /* Init */
  strcpy(file_name_rtn,file_path);

  /* Extrait le nom */
  for(i=(int)strlen(file_path); i>=0; i--)
    if(file_path[i] == '\\' || file_path[i] == '/')
      {
        strcpy(file_name_rtn,&file_path[i+1]);
        break;
      }
}
...
int64_t GetAddressValue(char *expression, int current_address, struct external **current_external_rtn, int *is_dum_label_rtn, int *is_fix_label_rtn, struct omf_segment *current_omfsegment)
{
  ...
  *is_fix_label_rtn = 0;               /* Par défaut, le label n'est pas ORG */
  ...
  if(current_label->line->is_fixed_address == 1)
    *is_fix_label_rtn = 1;
...
/******************************************************************************************************/
/*  BuildProjectFileName() :  Construit le nom du fichier associé à une directive ASM,DSK,PUT,PUTBIN. */
/******************************************************************************************************/
char *BuildProjectFileName(char *asm_dsk_put_name)
{
  int i;
  char *next_occur;
  char file_name[1024] = "";

  /* Isole le nom de fichier */
  for(i=strlen(asm_dsk_put_name); i>=0; i--)
    if(asm_dsk_put_name[i] == '/' || asm_dsk_put_name[i] == '\\')
      {
        strcpy(file_name,&asm_dsk_put_name[i+1]);
        break;
      }

  /* On va gérer les noms de fichiers contenant les informations #TypeAuxType */
  next_occur = strchr(file_name,'#');
  if(next_occur != NULL)
    *next_occur = '\0';
...
/*************************************************************************************************************/
/*  BuildProjectFilePath() :  Construit le chemin du fichier associé à une directive ASM,DSK,PUT,PUTBIN,USE. */
/*************************************************************************************************************/
char *BuildProjectFilePath(char *asm_dsk_put_use_name, char *directive)
{
  char file_path[1024];
  struct parameter *param;
  my_Memory(MEMORY_GET_PARAM,&param,NULL,NULL);
  ...
  /* Vide ? */
  if(strlen(asm_dsk_put_use_name) == 0)
    return(strdup(""));

  /** Pour les USE, on détecte le 4/... pour le remplacer par le Macro Folder Path **/
  if(!my_stricmp(directive,"USE") && asm_dsk_put_use_name[0] == '4' && asm_dsk_put_use_name[1] == '/')
    {
      strcpy(file_path,param->macro_folder_path);
      strcat(file_path,&asm_dsk_put_use_name[2]);
    }
  else
    {
      /** Est-ce déjà un chemin complet ? **/
      if(IsAbsolutePath(asm_dsk_put_use_name))
        strcpy(file_path,asm_dsk_put_use_name);             /* Windows / MacOS / Linux Path */
      else
        {
          /** Chemin relatif **/
          strcpy(file_path,param->project_folder_path);
          strcat(file_path,asm_dsk_put_use_name);
        }
    }

  /* Faut t'il ajouter un .s pour les fichiers source ? */
  if(!my_stricmp(directive,"ASM") || !my_stricmp(directive,"PUT") || !my_stricmp(directive,"USE"))
    if(my_IsFileExist(file_path) == 0)
      {
        /* Le fichier n'existe pas */
        if(strlen(file_path) > 2)
          if(my_stricmp(&file_path[strlen(file_path)-2],".s"))
            {
              /* Ajoute le .s au chemin complet */
              strcat(file_path,".s");
              if(my_IsFileExist(file_path) == 0)
                file_path[strlen(file_path)-2] = '\0';   /* On supprime le .s vu qu'il ne permet pas de résoudre le chemin du fichier :-( */
            }
      }

  /* Pour les DSK on supprime le '.L' s'il existe à la fin */
  if(!my_stricmp(directive,"DSK"))
    if(strlen(file_path) > 2)
      if(!my_stricmp(&file_path[strlen(file_path)-2],".L"))
        file_path[strlen(file_path)-2] = '\0';

  /* Nettoie le chemin du fichier */
  CleanUpFilePath(file_path);
...
{
  /* Est ce que le file_name est un chemin absolu ? */
  if(IsAbsolutePath(file_name) == 1)
    {
      strcpy(file_path_rtn,file_name);
      return;
    }

  /* Relative Path : Project Folder Path + File Name */
...
/*************************************************************************/
/*  BuildOutputFolderPath() :  Construit le chemin du dossier du projet. */
/*************************************************************************/
void BuildOutputFolderPath(char *source_file_path, char *output_folder_path_rtn)
{
  int i;

  /** Chemin Absolu **/
  if(IsAbsolutePath(source_file_path) == 1)
    strcpy(output_folder_path_rtn,source_file_path);
  else
    {
      /** Chemin Relatif : Current Folder + Relative Path **/
      getcwd(output_folder_path_rtn,1024);
      strcat(output_folder_path_rtn,FOLDER_SEPARATOR);
      strcat(output_folder_path_rtn,source_file_path);
    }

  /** Extrait le dossier du chemin **/
  for(i=(int)strlen(output_folder_path_rtn); i>=0; i--)
    if(output_folder_path_rtn[i] == '\\' || output_folder_path_rtn[i] == '/')
      {
        output_folder_path_rtn[i+1] = '\0';
        break;
      }
}
...
/******************************************************************************/
/*  IsAbsolutePath() :  Vérifie si le chemin du fichier est un chemin absolu. */
/******************************************************************************/
int IsAbsolutePath(char *file_path)
{
  /* Empty */
...
/***********************************************************/
/*  CleanUpFilePath() :  Nettoyage du chemin d'un fichier. */
/***********************************************************/
void CleanUpFilePath(char *file_path)
{
  int i, begin_index, current_index;
  char *next_occur;
  char *begin_suffix;
  char *end_prefix;
  ...
  /* Supprime le /. final */
  if(strlen(file_path) >= 2)
    if(!my_stricmp(&file_path[strlen(file_path)-2],"/."))
      file_path[strlen(file_path)-2] = '\0';

  /* Supprime le /.. final */
  if(strlen(file_path) >= 3)
    if(!my_stricmp(&file_path[strlen(file_path)-3],"/.."))
      file_path[strlen(file_path)-3] = '\0';

  /** On supprime les /./ du chemin **/
  next_occur = strstr(file_path,"/./");
  while(next_occur)
    {
      next_occur++;
      memmove(next_occur,next_occur+strlen("./"),strlen(next_occur+strlen("./"))+1);
      next_occur = strstr(file_path,"/./");
    }
  ...
  if(strstr(file_path,"/../") != NULL)
    {
      /** Détermine le début du chemin incompressible **/
#if defined(WIN32) || defined(WIN64)
      /* Windows */
      if(file_path[1] == ':')    /* c:\ */
        begin_index = 3;
      else if(file_path[0] == '/' && file_path[1] == '/')   /* \\<name>\*/
        {
          next_occur = strchr(&file_path[2],'/');
          if(next_occur == NULL)
            begin_index = -1;      /* Unknown format */
          else
            begin_index = (int) (next_occur - &file_path[0]) + 1;
        }
      else
        begin_index = -1;      /* Unknown format */
#else
      ...
      while(next_occur)
        {
          /** Cas particulier du début de chemin : /../ ou c:\..\ ou \\<server>\..\ **/
          current_index = (int) (next_occur - &file_path[0]);
          if(current_index < (begin_index-1))       /* Protection */
            break;
          if(current_index == (begin_index-1))
            {
              memmove(&file_path[begin_index],next_occur+strlen("/../"),strlen("/../")+1);
              next_occur = strstr(file_path,"/../");
              continue;
            }

          /** Supprime un niveau de répertoire : <prefix>/<folder>/../<suffix> => <prefix>/<suffix> **/
          current_index--;
          begin_suffix = next_occur + strlen("/../");
          end_prefix = NULL;
          for(i = current_index; i>=(begin_index-1); i--)
            if(file_path[i] == '/')
              {
                end_prefix = &file_path[i+1];
                break;
              }
          if(end_prefix == NULL)                /* Protection */
            break;
          memmove(end_prefix,begin_suffix,strlen(begin_suffix)+1);
  ...
  for(i=strlen(file_path); i>=0; i--)
    if(file_path[i] == '/')
      file_path[i] = FOLDER_CHAR_SEPARATOR;
}


/************************************************************/
/*  DecodeDirectiveTYP() :  Décode les alias Ascii des TYP. */
/************************************************************/
void DecodeDirectiveTYP(char *buffer_typ)
{
  int i;
  char *tab_ascii[] = {"BAD","BIN","TXT","DIR","ADB","AWP","ASP","GSB","TDF","BDF","SRC","OBJ","LIB","S16","RTL","EXE","PIF","TIF","NDA","CDA","TOL","DVR","LDF","FST","DOC","PNT","PIC","FON","PAS","CMD","BAS","VAR","REL","SYS",NULL};
  char *tab_hexa[]  = {"$01","$06","$04","$0F","$19","$1A","$1B","$AB","$AC","$AD","$B0","$B1","$B2","$B3","$B4","$B5","$B6","$B7","$B8","$B9","$BA","$BB","$BC","$BD","$BF","$C0","$C1","$C8","$EF","$F0","$FC","$FD","$FE","$FF",NULL};

  /* Identifie/Remplace les Alias connus */
  for(i=0; tab_ascii[i] != NULL; i++)
    if(!my_stricmp(buffer_typ,tab_ascii[i]))
      {
        strcpy(buffer_typ,tab_hexa[i]);
        return;
      }
}
...
/*******************************************************************/
struct parameter *mem_alloc_param(char *macro_folder_path)
{
  WORD one_word;
  ...
  /* Macro Folder Path */
  strcpy(param->macro_folder_path,macro_folder_path);
