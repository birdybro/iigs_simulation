/***********************************************************************/
/*                                                                     */
/*  a65816_Data.c : Module pour la génération du code objet.           */
/*                                                                     */
/***********************************************************************/
...
static void BuildOneDataLineSize(struct source_line *current_line, char *buffer_error_rtn, struct omf_segment *current_omfsegment)
{
  ...
  if(!my_stricmp(current_line->opcode_txt,"DA") || !my_stricmp(current_line->opcode_txt,"DW") || !my_stricmp(current_line->opcode_txt,"DDB") ||
  ...
  else if(!my_stricmp(current_line->opcode_txt,"DS"))
    {
      ...
      /** Cas particulier du DS \ et du DS \VALUE **/;
      if(tab_element[0][0] == '\\')
        {
          /** Taille de la partie Data => il faudra complèter jusqu'à la Page suivante ou une autre valeur **/
          current_line->nb_byte = 0xFFFF;   /* 4 F */
          
          /* On aligne */
          current_line->alignment = 256;      /* Par défaut, on aligne sur la Page suivante */
          if(strlen(&tab_element[0][1]) > 0)
            current_line->alignment = atoi(&tab_element[0][1]);

          /* Erreur */
          if(current_line->alignment <= 0 || current_line->alignment >= 65536)
            {
              mem_free_table(nb_element,tab_element);
              sprintf(buffer_error_rtn,"Wrong DS Format Data in Operand '%s', '%s' is not a valid alignment value",current_line->operand_txt,&tab_element[0][1]);
              return;
            }
        }
      else
