...
char *directive_list[] = 
{
  ...
  "LUP","--^",
  "ERR","DAT",
  "LNK","SAV","AST","CYC","EXP","LST","LSTDO","PAG","TTL","SKP","TR","KBD","PAU","SW","USR","AUX",   /* On ne fait rien avec ces Directives */
  NULL
};
  ...
  for(current_line=first_line; current_line; current_line=current_line->next)
    {
     ...
     if(strlen(current_line->label_txt) == 0 && strlen(current_line->opcode_txt) == 0 && strlen(current_line->operand_txt) == 0)
      ...
      /** Ligne DAT à laquelle on ajoute la Date sous forme de Texte **/
              ...
              if(current_item != NULL)
                {
                  ...
                  /* On repère les REL (mais on en tient pas compte si on est sur un project fixed address) */
                  if(!my_stricmp(current_line->opcode_txt,"REL") && current_omfproject->address_type != ADDRESS_TYPE_FIXED)
      ...
      for(i=1; i<=nb_global; i++)
        {
          ...
          if(found == 0)
            {
              /* On crée une ligne ENT avec le label en la placant après la ligne ENT Label1,Label2...*/
...
int ComputeLineAddress(struct omf_segment *current_omfsegment, struct omf_project *current_omfproject)
{
  ...
  int line_number, current_address, global_address, new_address, dum_address, nb_byte, has_previous_label, is_reloc, is_first_org, is_fixed_address;
  ...
  /* Init */
  if(current_omfproject->address_type == ADDRESS_TYPE_RELATIVE_OMF)
    {
      /* Le code OMF est relogeable : On assemble coimme si on était en $00/0000 */
      current_bank     = 0;
      current_address  = 0x0000;
      is_fixed_address = 0;
    }
  else if(current_omfproject->address_type == ADDRESS_TYPE_FIXED && current_omfproject->merge_fa_segments == 1)
    {
      /* L'adresse ORG est transmise de Segment en Segment qui s'enchainent au sein d'un même fichier */
    ...
    {
      /* Chaque Segment Fixed Address est autonome et crée son propre fichier */
      current_bank     = 0;
      ...
      if(current_line->type == LINE_DIRECTIVE && !my_stricmp(current_line->opcode_txt,"ORG"))
        {
          ...
          if(strlen(current_line->operand_txt) > 0)
            { 
              ...
              current_line->is_fixed_address = is_fixed_address;
              ...
              if(is_first_org == 1 && current_omfproject->address_type == ADDRESS_TYPE_FIXED && current_omfproject->merge_fa_segments == 0)
                {
          ...
          else    /* ORG */
            {
              ...
              if(current_omfproject->address_type == ADDRESS_TYPE_RELATIVE_OMF || current_omfproject->merge_fa_segments == 1)
                is_fixed_address = 0;
            }
        }
      ...
      if(current_line->is_dum == 1)
        {
          ...
          if(current_line->nb_byte == 0xFFFF)
            {
              /* Cas particulier des lignes DS \ : Alignement sur le prochain $100 ou la valeur indiquée */
              nb_byte = 0;
              if((dum_address % current_line->alignment) != 0)
                nb_byte = current_line->alignment - (dum_address % current_line->alignment);
      ...
      else
        {
          ...
          if(current_line->nb_byte == 0xFFFF)
            {
              /* Cas particulier des lignes DS \ : Alignement sur le prochain $100 ou sur la valeur indiquée */
              nb_byte = 0;
              if((current_address % current_line->alignment) != 0)
                nb_byte = current_line->alignment - (current_address % current_line->alignment);
              current_line->nb_byte = nb_byte;
...
struct source_line *DuplicateSourceLine(struct source_line *current_line)
{
  ...
  new_line->alignment = current_line->alignment;  
